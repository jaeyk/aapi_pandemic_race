...
biden_total_plot <- df %>%
  ...
  summarize(biden_support = mean(biden, na.rm = T),
            ci_high = ci.high(biden),
            ci_low = ci.low(biden)) %>%
  ggplot(aes(x = glue("{factor(wave_fac)} 2020"), 
             y = round(biden_support, 2),
         ymax = biden_support + ci_high, 
         ymin = biden_support - ci_low)) +
  geom_errorbar() +
  geom_col(alpha = 0.5) +
...
ggsave(here("outputs", "fig1_desc.png"), width = 13)
