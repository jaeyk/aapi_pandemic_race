...
df <- replace_na(df, list(lostjob = 0, 
                          ronaunfairasian = 0, 
                          incomereduc = 0,
                          rona.apa.mistreat = 0))
```
...
mean(subset(df, wave == 2)$rona.apa.mistreat, is.na = T)
```
...
wit_plot <- df %>%
  ...
  ggplot(aes(x = proxy, y = mean,
             ymax = mean + ci_high,
             ymin = mean - ci_low)) +
    geom_pointrange() +
    ylim(c(0, 1)) +
    labs(x = "Response to general discrimination Q in Wave 1", 
         y = "Response to witnessing COVID-19 discrimination Q in Wave 2") +
    scale_colour_brewer(palette = "Set1") +
    ggrepel::geom_text_repel(aes(label = paste0(round(mean, 2)*100, "%"))) +
        scale_y_continuous(labels = scales::percent)
