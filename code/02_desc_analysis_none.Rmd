...
var.list <- c(var.list, c("linkedfate", "idimport", "asnpride", "rona.behav.sneeze", "rona.behav.language", "rona.behav.walk", "rona.behav.transit", "rona.behav.other", "rona.behav.nochange", "rona.index", "X2020likelyvote", "biden", "DEM", "DEM.strong", "GOP", "GOP.strong", "unfair", "unfair_asian", "apa.responsible", "apa.have.rona", "apa.harassment", "whiteaffect", "blackaffect", "latinoaffect", "asianaffect"))
```
...
group_sum %>%
    #filter(wave_fac == "May") %>% 
    filter(str_detect(variable, "unfair")) %>%
    mutate(variable = recode(variable, 
 "unfair_asian" = "Experienced COVID discrimination due to race",
...
```{r}
summary(lm(df$apa.discrim.rona ~ df$unfair_asian))
```
...
mod2 <- lm(apa.discrim.rona ~ unfair_asian, data = subset(df, wave == 2))
...
lm(apa.discrim.rona ~ gendiscrim, data = df) %>%
    tidy()
```
...
wit_plot <- df %>%
  filter(wave != 1) %>%
  group_by(proxy, wave) %>%
  summarize(mean = mean(unfair_asian, na.rm = TRUE),
            ci_high  = ci.high(unfair_asian, na.rm = TRUE),
            ci_low = ci.low(unfair_asian, na.rm = TRUE)) %>%
  ggplot(aes(x = proxy, y = mean, col = factor(wave),
             ymax = mean + ci_high,
             ymin = mean - ci_low)) +
    geom_pointrange() +
...
```{r eval = FALSE}
summary(lm(unfair_asian ~ prior, data = df))
```
