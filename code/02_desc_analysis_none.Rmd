...
df %>%
  ...
  group_by(gendiscrim, party.id) %>%
  dplyr::summarize(n = n()) %>%
    mutate(prop = n / sum(n),
           prop = round(prop,2)) %>%
  ggplot(aes(x = gendiscrim, y = prop, group = 1,
               fill = factor(party.id))) +
    geom_col(position = "fill") +
    scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
    scale_fill_manual(values = c("blue", "purple", "red")) +
  labs(y = "Proportion", x = "General discrimination perception", fill = "Party ID")
