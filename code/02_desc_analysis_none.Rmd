...
group_sum %>%
    ...
    ggplot(aes(x = glue("{factor(wave_fac)} 2020"), y = mean,
        ...
        geom_col(alpha = 0.5) +
        ggrepel::geom_text_repel(aes(label = round(mean, 2), col = "red", size = 2)) +
        facet_wrap(~variable) +
        labs(
...
covid_dis <- (0.23 + 0.2)/2 
gen_dis <- (0 + abs(-0.02))/2
...
plot_w123 %>%
    ggplot(aes(x = factor(proxy), y = mean,
        ...
        geom_errorbar(position = dodge, width = 0.25) +
        geom_col(alpha = 0.5) +
        ggrepel::geom_text_repel(aes(label = round(mean, 2), col = "red")) +
