...
```{r}
table_df <- df %>%
  dplyr::summarize(female = mean(male %>% unclass() - 1),
...
male_dist <- data.frame(male = c(1, 0),
                        Freq = nrow(df) * c(0.47, 1-0.24))

# raking 

df_raked <- rake(design = df_unweighted,
                 sample.margins = list(~chinese, ~indian, ~male, ~usborn),
                 population.margins = list(chinese_dist, indian_dist, male_dist, usborn_dist))
