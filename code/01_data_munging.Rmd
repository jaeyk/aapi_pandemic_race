...
```{r}
w1 <- long_df %>%
  select(matches("W1|response|respondent|pid")) %>%
...
w2 <- long_df %>%
  select(matches("W2|response|respondent|pid")) %>%
...
w3 <- long_df %>%
  select(matches("W3|response|respondent|pid")) %>%
...
long_df <- bind_rows(w1, w2, w3)

# Check
subset(long_df, wave == 2)$expdiscrim
```

## Final merging 

```{r}
long_complete <- long_df %>%
